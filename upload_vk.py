...
def upload_to_vk(video_path, description="", title=""):
    ...
    try:
        ...
        # Prepare message
        message = description if description else "🐻 Добрая сказка для детей\n\n#СказкиДляДетей #ДобрыеСказки #Животные"
        
        # Ensure message is not empty (VK requirement)
        if not message.strip():
            message = "🐻 Добрая сказка для детей!"
        ...
        video = upload.video(
            ...
            name=title or 'Добрая сказка для детей',
...
def main():
    ...
    description = sys.argv[2] if len(sys.argv) > 2 else "Добрая сказка для детей 🐻"
    title = sys.argv[3] if len(sys.argv) > 3 else "Добрая сказка для детей"
