...
def upload_to_facebook(video_path, description):
    ...
    if not access_token:
        error_msg = "âŒ FACEBOOK_ACCESS_TOKEN or FB_ACCESS_TOKEN not set in environment variables"
        print(f"[facebook] {error_msg}")
        raise ValueError(error_msg)
    
    if not page_id:
        error_msg = "âŒ FACEBOOK_PAGE_ID or FB_PAGE_ID not set in environment variables"
        print(f"[facebook] {error_msg}")
        raise ValueError(error_msg)
    ...
    try:
        # Extract a title from description (first line before any double newline)
        title_from_desc = description.split('\n\n')[0][:100] if description else "Ð”Ð¾Ð±Ñ€Ð°Ñ ÑÐºÐ°Ð·ÐºÐ° Ð´Ð»Ñ Ð´ÐµÑ‚ÐµÐ¹"
