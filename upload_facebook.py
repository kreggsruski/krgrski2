...
def upload_to_facebook(video_path, description):
    ...
    if not access_token:
        error_msg = "❌ FACEBOOK_ACCESS_TOKEN or FB_ACCESS_TOKEN not set in environment variables"
        print(f"[facebook] {error_msg}")
        raise ValueError(error_msg)
    
    if not page_id:
        error_msg = "❌ FACEBOOK_PAGE_ID or FB_PAGE_ID not set in environment variables"
        print(f"[facebook] {error_msg}")
        raise ValueError(error_msg)
    ...
    try:
        ...
        title_from_desc = description.split('\n\n')[0][:100] if description else "Добрая сказка для детей"
